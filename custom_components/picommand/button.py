...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for node_id in coordinator.data["nodes"]:
        entities += [
            PiCommandRebootButton(coordinator, node_id),
            PiCommandUpdateButton(coordinator, node_id),
        ]
    async_add_entities(entities)

    async def handle_run_command(call):
        ...
        try:
            ...
            _LOGGER.info("PiCommand [%s] exit=%s", node_id, result.get("exit_code"))
    ...
    hass.services.async_register(
        DOMAIN, "run_command", handle_run_command,
        schema=vol.Schema({
...
class PiCommandRebootButton(PiCommandBaseButton):
    ...
    @property
    def unique_id(self):
        return f"picommand_{self._node_id}_reboot"

    @property
    def name(self):
        return f"{self.node.get('display_name', self._node_id)} Reboot"
...
class PiCommandUpdateButton(PiCommandBaseButton):
    ...
    @property
    def unique_id(self):
        return f"picommand_{self._node_id}_update"

    @property
    def name(self):
        return f"{self.node.get('display_name', self._node_id)} Update"
